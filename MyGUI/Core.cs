...
namespace MyGUI.Utilities
{
    ...
    public class AbstractCollection<T> : List<T> where T : INameable, IFocusable
    {
        ...
        public T Get(params Focus[] focusStatus)
        {
			for (int i = 0; i < focusStatus.Length; i++)
			{
				try
				{
					return this.Where(t => t.FocusStatus == focusStatus[i]).Single();
				}
				catch { }
			}
    ...
    public abstract class PrimitiveComponent : IEntity
    {
        ...
        public T GetParent<T>() where T : IEntity
        {
            if (parent is T)
            {
                return (T)parent;
	...
	public abstract class ComponentComplex : PrimitiveComponent
	{
		private Focus focusStatus = Focus.NoFocus;
		...
		public virtual AbstractCollection<PrimitiveComponent> Collection { get; set; }
			= new AbstractCollection<PrimitiveComponent>();
	}
	...
	public abstract class Container : IEntity
    {
        ...
        public virtual AbstractCollection<IEntity> Collection { get; set; } = new AbstractCollection<IEntity>();
    ...
    public abstract class Group<TItem> : Container where TItem : IEntity
    {
        public new AbstractCollection<TItem> Collection { get; set; } = new AbstractCollection<TItem>();
    }
